classdef Bot < handle
    ...
    methods
        ...
        function open = openRight(obj, distRight)
            open = distRight > obj.maze.CELL_SIZE;
            return;
            ...
            odo = obj.odo;
            usx = odo.pose(1) + odo.forward(1) * hw.yUS + odo.right(1) * hw.xUS;
            usy = odo.pose(2) + odo.forward(2) * hw.yUS + odo.right(2) * hw.xUS;
            BUFFER = -CELL_SIZE / 4;
            wx = usx + odo.right(1) * (distRight + BUFFER);
            wy = usy + odo.right(2) * (distRight + BUFFER);
            cellY = floor(odo.pose(2) / CELL_SIZE) * CELL_SIZE;
            cellX = floor(odo.pose(1) / CELL_SIZE) * CELL_SIZE;
            switch mod(round(obj.targetAngle / 90), 4)
                case 0 % going east
                    open = Maze.intersects(usx, usy, wx, wy, cellX, cellY, cellX + CELL_SIZE, cellY);
                case 1 % going south
                    open = Maze.intersects(usx, usy, wx, wy, cellX, cellY, cellX, cellY + CELL_SIZE);
                case 2 % going west
                    open = Maze.intersects(usx, usy, wx, wy, cellX, cellY + CELL_SIZE, cellX + CELL_SIZE, cellY + CELL_SIZE);
                case 3 % going north
                    open = Maze.intersects(usx, usy, wx, wy, cellX + CELL_SIZE, cellY, cellX + CELL_SIZE, cellY + CELL_SIZE);
            end
        end
        ...
        function drift = getDrift(obj)
            ...
            switch mod(round(obj.targetAngle / 90), 4)
                case 0 % going east
                    targetAngle = targetAngle + (CELL_SIZE - obj.WALL_DIST - mod(odo.pose(2), CELL_SIZE)) * FACTOR;
                case 1 % going south
                    targetAngle = targetAngle + (mod(odo.pose(1), CELL_SIZE) - obj.WALL_DIST) * FACTOR;
                case 2 % going west
                    targetAngle = targetAngle + (mod(odo.pose(2), CELL_SIZE) - obj.WALL_DIST) * FACTOR;
                case 3 % going north
                    targetAngle = targetAngle + (CELL_SIZE - obj.WALL_DIST - mod(odo.pose(1), CELL_SIZE)) * FACTOR;
            end
            drift = odo.pose(3) - max(obj.targetAngle, min(obj.targetAngle + 5, targetAngle));
        end
                    ...
                    switch obj.state
                        case "stateStraight"
                            ...
                            if obj.openRight(rightDist)
                                ...
                                switch mod(round(obj.targetAngle / 90), 4)
                                    case 0 % going north, turned east
                                        obj.pCell = cellX;
                                        turnRadius = max(hw.WHEEL_DIST, obj.WALL_DIST - mod(odo.pose(2), maze.CELL_SIZE));
                                    case 1 % going west, turned north
                                        obj.pCell = cellY;
                                        turnRadius = max(hw.WHEEL_DIST, obj.WALL_DIST - (maze.CELL_SIZE - mod(odo.pose(1), maze.CELL_SIZE)));
                                    case 2 % going south, turned west
                                        obj.pCell = cellX;
                                        turnRadius = max(hw.WHEEL_DIST, obj.WALL_DIST - (maze.CELL_SIZE - mod(odo.pose(2), maze.CELL_SIZE)));
                                    case 3 % going east, turned south
                                        obj.pCell = cellY;
                                        turnRadius = max(hw.WHEEL_DIST, obj.WALL_DIST - mod(odo.pose(1), maze.CELL_SIZE));
                                end
                                if obj.pState == "stateBackward"
                                    turnRadius = hw.WHEEL_DIST;
                                else
                                    turnRadius = hw.WHEEL_DIST;
                                end
                            ...
                            if abs(rightDist - obj.pRightDist) < 2 && obj.wasStraight
                        ...
                        case "stateBackward"
                            ...
                            powerR = Bot.inter(angle, [90, -100; 170, -20; 180, 0; 190, 20]) * obj.AUTO_SPEED / 100;
                            hw.move(powerL, powerR);
                            if abs(powerL) < 10 && abs(powerR) < 10
                                obj.changeState("stateStraight");
                                hw.beep();
                            end
                        case "stateRight"
                            angle = odo.pose(3) - (obj.targetAngle - 90);
                            power = obj.inter(angle, [60, 100; 90, 0; 100, -20]) * obj.AUTO_SPEED / 100;
                            hw.steer(power, turnRadius);
                            if abs(power) < 10
                        ...
                        case "stateForward"
                            ...
                            switch mod(round(obj.targetAngle / 90), 4)
                                case 0 % going east
                                    if odo.pose(1) + hw.yUS > (obj.pCell + 1) * maze.CELL_SIZE + BUFFER
                                        obj.changeState("stateStraight");
                                        hw.beep();
                                    end
                                case 1 % going south
                                    if odo.pose(2) - hw.yUS < obj.pCell * maze.CELL_SIZE - BUFFER
                                        obj.changeState("stateStraight");
                                        hw.beep();
                                    end
                                case 2 % going west
                                    if odo.pose(1) - hw.yUS < obj.pCell * maze.CELL_SIZE - BUFFER
                                        obj.changeState("stateStraight");
                                        hw.beep();
                                    end
                                case 3 % going north
                                    if odo.pose(2) + hw.yUS > (obj.pCell + 1) * maze.CELL_SIZE + BUFFER
                                        obj.changeState("stateStraight");
                                        hw.beep();
                                    end
                            end
                        case "stateSpin"
                            angle = odo.pose(3) - (obj.targetAngle - 180);
                            power = Bot.inter(angle, [90, 100; 160, 70; 180, 0; 190, -20]) / 100 * obj.AUTO_SPEED;
                            hw.steer(power, 0);
                            if abs(power) < 20
                                hw.stop();
